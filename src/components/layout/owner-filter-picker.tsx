...
export function OwnerFilterPicker() {
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex shrink-0 items-center gap-1 rounded-full border border-accent bg-accent px-2.5 py-1.5 text-xs font-medium text-accent-foreground shadow-sm transition-colors cursor-pointer"
      >
        ...
        <div className="absolute end-0 top-full z-30 mt-1.5 min-w-32 overflow-hidden rounded-xl border border-border bg-surface py-1 shadow-lg">
          ...
          {familyMembers.map((member) => (
            <button
              ...
              className={cn(
                "flex w-full items-center gap-1.5 px-3 py-2 text-start text-sm font-medium cursor-pointer hover:bg-surface-2",
                ownerFilter === member.user_id ? "text-accent" : "text-foreground",
              )}
            >
              {member.display_name}
            </button>
